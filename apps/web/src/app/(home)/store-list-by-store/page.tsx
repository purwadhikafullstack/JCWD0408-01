            ...
            <div className="lg:ml-64">
                ...
                <div className="flex flex-wrap gap-7 px-10 py-10">
                {
                        data?.store.map((item, key) => {
                            return (
                                <Link href={`/store-list-by-store/admin-dashboard/${item.store_id}`} >
                                    <CartStorebyStore key={key} nama={item.store_name} nama_pengelola="ADMIN SAAT INI" created_at={item.created_at} />
                                </Link>
                            )
                        })
                    }
